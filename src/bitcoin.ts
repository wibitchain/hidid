...
interface IBitcoinServiceConifg extends IDsLinkBitcoinConfig {
...
const configFilePath = process.env.DsLink_BITCOIN_CONFIG_FILE_PATH || '../json/bitcoin-config.json';
...
const port = process.env.DsLink_BITCOIN_PORT || config.port;
...
let blockchainService: DsLinkBitcoinProcessor;

try {

blockchainService = new DsLinkBitcoinProcessor(config);



// DsLink_TEST_MODE enables unit testing of this file by bypassing blockchain service initialization.

if (process.env.DsLink_TEST_MODE === 'true') {
...
console.log(`DsLink-Bitcoin node running on port: ${port}`);
...
console.error(`DsLink-Bitcoin node initialization failed with error: ${error}`);
...
console.log(DsLinkBitcoinProcessor.generatePrivateKey('testnet'));

process.exit(1);

}

console.info('DsLink bitcoin service configuration:');
